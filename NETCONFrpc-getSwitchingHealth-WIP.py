...
# 2022-05-21
# derivative of NETCONFrpc-getInterfaceData.py
...
def check_int_crc(mysqlenv, devicename, xmldata):
    ...
    for interface in xmldata:
        ...
        print(f'Interface: {interface.find("name").text} has {int_crc} CRC errors')
        total_device_crcs += int_crc
    print(total_device_crcs)
...
WHERE hostname = "{devicename}"
""" 
                MySQLExecute.executesql(mysqlenv, sql)
                sendWebexMessage.sendMessage(ReadEnvironmentVars.read_config_file("switchalerts_webexroomid"),f'{devicename} has increasing CRC counts: difference of {total_device_crcs - results[0][1]}')
