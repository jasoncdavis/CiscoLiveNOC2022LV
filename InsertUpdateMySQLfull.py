...
v3      2022-0607   Quick rewrite to allow for CSV import with more params 
Exchange
...
def insertsql(serverparams,sql_values):
    ...
    cursor=db.cursor()
    SQL = f"""INSERT INTO {serverparams["database"]}.inventory 
    (hostname, mgmt_ip_address, serial_number, device_type, device_group, \
     model, source, software_version, location, contacts, do_ping) 
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 1) 
    ON DUPLICATE KEY UPDATE hostname=VALUES(hostname), serial_number=VALUES(serial_number), \
     device_type=VALUES(device_type), device_group=VALUES(device_group), \
     model=VALUES(model), source=VALUES(source), software_version=VALUES(software_version), \
     location=VALUES(location), contacts=VALUES(contacts)
    """
    try:
